...
# def populate_foods(request):
#     PATH = "C:\\Program Files (x86)\\chromedriver.exe"
#     driver = webdriver.Chrome(PATH)
#
#     driver.get("https://www.tesco.com/groceries/en-GB/trolley")
#
#     links = []
#
#     try:
#         email = driver.find_element(by=By.ID, value="email")
#         email.send_keys("bobiyonkov01@gmail.com")
#
#         password = driver.find_element(by=By.ID, value="password")
#         password.send_keys("B!787zYcSdRvSyu")
#         password.send_keys(Keys.RETURN)
#
#         product_list = WebDriverWait(driver, 10).until(
#             EC.presence_of_element_located((By.CLASS_NAME, "product-list-container"))
#         )
#         products = product_list.find_elements(by=By.CLASS_NAME, value="product-list--list-item")
#         for product in products:
#             title = product.find_element(by=By.CLASS_NAME, value="product-details--content")
#             title_a = title.find_element(by=By.TAG_NAME, value="a")
...
#         for link in links:
#             driver.get(link)
#             name = driver.find_element(by=By.CLASS_NAME, value="product-details-tile__title").text
#             print(name)
#             price = driver.find_element(by=By.CLASS_NAME, value="price-per-sellable-unit").text
#             price = float(price.replace("£", "").strip())
#             description = driver.find_element(by=By.CLASS_NAME, value="product-info-block__content-item").text.strip()
#             serving = ""
#             cals = 1.0
#             fat = 0.0
#             carb = 0.0
#             protein = 0.0
#             if driver.find_elements(by=By.CSS_SELECTOR, value="table"):
#                 nutrition_table = driver.find_element(by=By.CLASS_NAME, value="product__info-table")
#                 tbody = nutrition_table.find_element_by_tag_name('thead')
#                 table_rows = tbody.find_elements(by=By.TAG_NAME, value="tr")
#                 print(table_rows[0].find_elements(by=By.TAG_NAME, value="th"))
#                 if len(table_rows[0].find_elements(by=By.TAG_NAME, value="th")) > 2:
#                     serving = nutrition_table.find_elements(by=By.TAG_NAME, value="th")[2].text
#                     if "per" in serving.lower():
#                         serving = serving.lower().replace("per", "").strip()
#                     tbody = nutrition_table.find_element_by_tag_name('tbody')
#                     table_rows = tbody.find_elements(by=By.TAG_NAME, value="tr")
#                     for row in table_rows:
#                         if row.find_elements(by=By.TAG_NAME, value="td")[0].text == "Energy":
#                             cals = row.find_elements(by=By.TAG_NAME, value="td")[2].text
#                             cals = re.sub(r'^.*?/', '/', cals).replace("/", "").strip()
#                             cals = cals.replace("kJ", "")
#                             cals = float(cals.replace("kcal", "").strip())
#                         elif row.find_elements(by=By.TAG_NAME, value="td")[0].text == "Fat":
#                             fat = row.find_elements(by=By.TAG_NAME, value="td")[2].text
#                             fat = re.findall(r"[-+]?(?:\d*\.\d+|\d+)", fat)[0]
#                         elif row.find_elements(by=By.TAG_NAME, value="td")[0].text == "Carbohydrate":
#                             carb = row.find_elements(by=By.TAG_NAME, value="td")[2].text
#                             carb = re.findall(r"[-+]?(?:\d*\.\d+|\d+)", carb)[0]
#                         elif row.find_elements(by=By.TAG_NAME, value="td")[0].text == "Protein":
#                             protein = row.find_elements(by=By.TAG_NAME, value="td")[2].text
#                             protein = re.findall(r"[-+]?(?:\d*\.\d+|\d+)", protein)[0]
#             else:
#                 top = driver.find_elements(by=By.CLASS_NAME, value="top")
#                 cals = top[0].find_element(by=By.CLASS_NAME, value="value").text
#                 cals = re.sub(r'^.*?J', 'J', cals).replace("J", "").strip()
#                 cals = float(cals.replace("kcal", ""))
#                 fat = top[1].find_element(by=By.CLASS_NAME, value="value").text
#                 fat = fat.replace("g", "").strip()
#                 gda = driver.find_element(by=By.CLASS_NAME, value="gda")
#                 serving = gda.find_element(by=By.CLASS_NAME, value="title").text
#                 missing_info_links.append(link)
#
#             uses = driver.find_element(by=By.ID, value="uses")
#             uses_value = uses.find_element(by=By.TAG_NAME, value="p").text
#             uses_value = [float(s) for s in uses_value.split() if s.isdigit()][0]
#             price_per_serving = price / uses_value
#             price_per_100cal = round((price_per_serving / cals) * 100, 2)
#             Food.objects.create(name=name, price=price, description=description, serving_size=1, unit=serving,
#                                 servings_per_container=uses_value, calories=cals, fat=fat, carb=carb, protein=protein,
#                                 price_per_serving=round(price_per_serving, 2), price_per_100cal=price_per_100cal)
#     finally:
#         driver.quit()
#     print(missing_info_links)
